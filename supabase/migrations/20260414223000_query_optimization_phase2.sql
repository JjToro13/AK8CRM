...
create index if not exists idx_calls_operation_start_time
  on public.calls using btree (operation_id, start_time desc)
  where operation_id is not null;

create index if not exists idx_calls_agent_start_time
  on public.calls using btree (agent_id, start_time desc);

create index if not exists idx_clients_assigned_operation_created
...
create index if not exists idx_clients_source_trgm
  on public.clients using gin (source gin_trgm_ops)
  where source is not null;

create index if not exists idx_clients_serial_trgm
