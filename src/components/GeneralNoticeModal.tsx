// GeneralNoticeModal.tsx - Un modal genérico para mostrar avisos importantes a los usuarios, con variantes de estilo para info, warning y danger. Permite opcionalmente guardar en localStorage que el usuario ya vio el aviso para no mostrarlo de nuevo.
...
type GeneralNoticeModalProps = {
  ...
  variant?: Variant;
  /** Si lo pasás, se guarda en localStorage y no vuelve a mostrarse */
  dismissKey?: string;
...
export default function GeneralNoticeModal({
  ...
  useEffect(() => {
    if (open && !dismissed) setShouldRender(true);
  }, [open, dismissed]);

  if (!open || dismissed || !shouldRender) return null;
